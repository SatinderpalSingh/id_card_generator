...
Class Master extends DBConnection {
	...
	function save_template(){
		...
		foreach($_POST as $k =>$v){
			if(in_array($k,array('name','width','height','template_code'))){
				if(!empty($data)) $data .=",";
				$v = trim($v);
				$data .= " `{$k}`='{$v}' ";
			}
		...
		if(empty($id)){
			$sql = "INSERT INTO `id_format` set $data ";
		}else{
			$sql = "UPDATE `id_format` set $data where id = '{$id}' ";
		}
		...
		if($save){
			...
			$sql = $this->conn->query("UPDATE `id_format` set $data where id = '{$tid}' ");
			$this->capture_err();
			$decoded = base64_decode(str_replace(' ','+',explode(',', $template_image)[1]));
			$img = imagecreatefromstring($decoded);
			$img = imagescale($img , ($width * 96), ($height * 96));
			$template_image = imagepng($img,base_app.$img_src);
			$resp['status'] = 'success';
			if(empty($id))
			$this->settings->set_flashdata('success',' ID Format Successfully saved.');
			else
			$this->settings->set_flashdata('success',' ID Format Successfully updated.');
		}else{
			$resp['status'] = 'failed';
		}
	...
	function delete_template(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `id_format` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Template successfully deleted.");
			if(is_file(base_app.'uploads/templates/template_'.$id.'.png')){
				unlink(base_app.'uploads/templates/template_'.$id.'.png');
			}
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
	...
	function save_generate(){
		...
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id','id_ss'))){
				$v = trim($v);
				if(!empty($data)) $data .=",";
				$data .= " `{$k}`='{$v}' ";
			}
		}
		$check = $this->conn->query("SELECT * FROM `generated_id` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " File Name already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `generated_id` set {$data} ";
		}else{
			$sql = "UPDATE `generated_id` set {$data} where id = '{$id}' ";
		}
		$save = $this->conn->query($sql);
		if($save){
			$gid = empty($id) ? $this->conn->insert_id : $id ;
			$img_src = 'uploads/generated/'.$name.'.png';
			$old_file = "";
			if($id > 0)
			$old_file = $this->conn->query("SELECT image_path FROM `generated_id` where id = '{$gid}' ")->fetch_array()['image_path'];
			$this->capture_err();
			if(!empty($old_file) && is_file(base_app.$old_file)){
				unlink(base_app.$old_file);
			}
			
			$decoded = base64_decode(str_replace(' ','+',explode(',', $id_ss)[1]));
			$img = imagecreatefromstring($decoded);
			$template_image = imagepng($img,base_app.$img_src);
			$sql = $this->conn->query("UPDATE `generated_id` set image_path = '{$img_src}' where id = '{$gid}' ");
			$this->capture_err();
			
			$resp['status'] = 'success';
			$resp['generated_url'] = validate_image($img_src);
			if(empty($id))
			$this->settings->set_flashdata('success',' Generated ID Successfully saved.');
			else
			$this->settings->set_flashdata('success',' Generated ID Successfully updated.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
	...
	function delete_id(){
		extract($_POST);
		$qry = $this->conn->query("SELECT image_path FROM `generated_id` where id = '{$id}' ");
		$img_path = $qry->num_rows > 0 ? $qry->fetch_array()['image_path'] : '';
		$del = $this->conn->query("DELETE FROM `generated_id` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Generated successfully deleted.");
			if(!empty($img_path) && is_file(base_app.$img_path)){
...
switch ($action) {
	...
	break;
	case 'save_generate':
		echo $Master->save_generate();
	break;
	case 'delete_id':
		echo $Master->delete_id();
	break;
